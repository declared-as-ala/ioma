...
async function main() {
  ...
  const isUpdateImages = args.includes("--update-images");
  ...
  for (const item of UAE_PRICE_LIST) {
    ...
    const slug = item.handle || item.name.toLowerCase().replace(/[^a-z0-9]+/g, "-");
    ...
    const titleAr = item.name; // Arabic title representation
    ...
    const productPayload: any = {
      ...
      shortBenefit: {
        en: `High-performance ${item.name} for visible results.`,
        fr: `Soin haute performance ${item.name} aux résultats prouvés.`,
        ar: `عناية فائقة الفعالية ${item.name} بنتائج مثبتة.`,
      },
  ...
  // 8. Clean up obsolete demo products if requested
  if (isApply) {
    const activeSlugs = [
